...
- **i18n:** RU/EN. The split follows one rule — **user-facing text is bilingual; model-facing text is single-language (English), because the model bridges languages itself.**
  ...
  - **`settings.uiLanguage`** (shared field, `WB_HELPER_LANGUAGE` env, first-run only) drives the UI **and** the few server strings that reach the user: the system-prompt language directive (`LANG_DIRECTIVE` in `chats.ts` — tells the model to **mirror the user's message language**), the welcome/fallback/checkpoint messages and `getExtraSystemMsgs` (localized via the `L()` helper in `index.ts`), and the default chat title / context suffix (`ChatStore` resolves it via a `getLang` callback).
  ...
  - The `[Система]` prefix stays a fixed protocol sentinel across languages (stripped by `api.ts` before display).
