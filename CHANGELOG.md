...
### Fixed
- **Модель отвечала на неправильном языке.** Две причины: (1) системный промпт
  с языковой директивой запекался один раз при создании чата — после смены
  языка в Настройках существующие чаты сохраняли старый язык; теперь ведущий
  system-turn пересобирается под текущий язык на каждой отправке. (2) Языковая
  директива была одной строкой в хвосте огромного русского системного промпта,
  и слабые модели её игнорировали, якорясь на русском. Директива вынесена в
  начало промпта отдельным приоритетным блоком и переписана жёстко: «отвечай
  строго на языке интерфейса, это перекрывает язык инструкций ниже».
