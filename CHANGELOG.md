...
- **Системный промпт: `sn` принимает IP/hostname/host:port.** Раньше
  правило «`sn` — буквенно-цифровой серийник вида A25NDEMJ» вступало в
  конфликт с manual-add'ом по IP — модель видела `31.185.4.250` в
  контексте, но колебалась использовать его как `sn`. Добавлено явное
  разрешение передавать строку контекста как есть.
