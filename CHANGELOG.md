...
### Added
- **Английский язык (RU/EN).** Переключатель в Настройках → «Интерфейс»,
  автодетект по языку браузера, env `WB_HELPER_LANGUAGE`. UI полностью
  локализован через лёгкий модуль `src/web/i18n.ts` (без vue-i18n).
  Ассистент **определяет язык пользователя и отвечает на нём** (директива в
  системном промпте). Скиллы и описания инструментов переведены на английский
  единым набором — это инструкции для модели, поэтому без дублирования по
  языкам (модель сама переводит ответ пользователю).
