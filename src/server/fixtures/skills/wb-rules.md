...
Все операции с файлами `/etc/wb-rules/*.js` — только через `wbrules/Editor`. **НЕ** используй `write_file` + `systemctl restart wb-rules` — RPC валидирует и перезагружает сам.

- `mqtt_rpc(sn, "wbrules", "Editor", "List", {})` — список правил
- `mqtt_rpc(sn, "wbrules", "Editor", "Load", {path})` — прочитать
- `mqtt_rpc(sn, "wbrules", "Editor", "Save", {path, content})` — сохранить
- `mqtt_rpc(sn, "wbrules", "Editor", "Remove", {path})` — удалить
- `mqtt_rpc(sn, "wbrules", "Editor", "Rename", {path, new_path})` — переименовать

**Отключить правило без удаления** — добавить `return; // wb-la-disabled` первой строкой внутри каждого `then: function(...) {`, сохранить через Save. Движок перезагрузит файл, правило перестанет срабатывать. Включить обратно — удалить эту строку, сохранить.
...
**Удалять правило — только с явным подтверждением пользователя.** Никогда не вызывай Remove без отдельного «да, удали».
