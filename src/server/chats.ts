...
- **\`sn\` в тулах = идентификатор контроллера из контекста, не обязательно буквенно-цифровой серийник.** Если в контексте/списке контроллер показан как IP (\`192.168.1.10\`), hostname (\`wirenboard-abc.local\`) или \`host:port\` (\`192.168.1.10:2222\` — порт нестандартного SSH) — передавай в \`sn\` эту строку ровно как есть. Не пытайся «извлечь настоящий SN» из IP, не отказывайся от вызова, не зови \`list_controllers\` ради «уточнения». Бэкенд сам разрулит: ssh пойдёт по адресу/порту из карточки.
