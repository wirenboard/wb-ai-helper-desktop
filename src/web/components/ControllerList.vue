...
function statusOf(c: Controller): { cls: string; text: string } {
  if (c.reachable === true) return { cls: 'ok', text: t('controllers.online') }
  if (c.reachable === false) return { cls: 'bad', text: t('controllers.offline') }
...
<template>
  <aside class="sidebar right" :class="{ collapsed: !open }">
    <div class="sidebar-header">
      <button class="ghost collapse-btn" :title="open ? t('common.collapse') : t('common.expand')" @click="emit('toggle-panel')">
        {{ open ? '›' : '‹' }}
      </button>
      <template v-if="open">
        <span>{{ t('controllers.list') }}</span>
        <button @click="emit('rescan')" :disabled="scanning" :title="scanning ? t('controllers.scanning') : t('controllers.rescan')" :style="scanning ? 'animation: spin 1s linear infinite' : ''">↻</button>
      </template>
    </div>
    <template v-if="open">
    <div class="sidebar-body">
      <div v-if="!controllers.length" class="empty">
        {{ t('controllers.empty') }}
        <span class="kbd">wirenboard-SN.local</span> {{ t('controllers.emptyMdns') }}
      </div>

      <div v-else class="row" style="margin-bottom:8px;justify-content:space-between">
        <span class="small muted">{{ t('controllers.countInNet', { n: controllers.length }) }}</span>
        <span class="row">
          <button class="small" @click="emit('select-all')">{{ t('controllers.all') }}</button>
          <button class="small" @click="emit('clear')">{{ t('controllers.clearSel') }}</button>
        </span>
      </div>
      ...
      >
        ...
        <div class="meta">
          ...
          <div class="host" :title="c.port ? t('controllers.sshPortTooltip', { host: c.host, port: c.port }) : c.host">
          ...
          <div class="row" style="justify-content:space-between;margin-top:2px">
            ...
            <span class="small muted">{{ c.source === 'manual' ? t('controllers.manual') : t('controllers.mdns') }}</span>
          </div>
          <div class="actions" @click.stop>
            <a
              ...
              target="_blank"
              rel="noopener"
              :title="t('controllers.webUi')"
              :aria-label="t('controllers.webUi')"
            >
            ...
            <button
              class="icon-action"
              :title="t('controllers.sshTerminal')"
              :aria-label="t('controllers.sshTerminal')"
            ...
            <button
              ...
              :title="t('controllers.removeFromList')"
    ...
    <div class="sidebar-footer">
      <div class="spread">
        <input
          v-model="manualHost"
          :placeholder="t('controllers.manualPh')"
          :title="t('controllers.manualExamples')"
          @keydown.enter="add"
        />
